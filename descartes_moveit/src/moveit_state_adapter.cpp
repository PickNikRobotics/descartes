...
bool MoveitStateAdapter::getIK(const Eigen::Affine3d& pose, std::vector<double>& joint_pose) const
{
  ...
  if (robot_state_->setFromIK(joint_group_, tool_pose, tool_frame_))
  {
    ...
    {
      logDebug("MoveitStateAdapter.getIK: Robot joint pose is invalid");
    }
    else
    {
      rtn = true;
...
bool MoveitStateAdapter::getAllIK(const Eigen::Affine3d& pose, std::vector<std::vector<double> >& joint_poses) const
{
  ...
  logDebug("MoveitStateAdapter.getAllIK: Utilizing an min. difference of %f between IK solutions", epsilon);
  joint_poses.clear();
    ...
    if (getIK(pose, joint_pose))
    {
      ...
      else
      {
        ...
        for (joint_pose_it = joint_poses.begin(); joint_pose_it != joint_poses.end(); ++joint_pose_it)
        {
          ...
          {
            logDebug("MoveitStateAdapter.getAllIK: Found matching, potential solution is not new");
            match_found = true;
            break;
          }
        }
  ...
  logDebug("MoveitStateAdapter.getAllIK: Found %lu joint solutions out of %lu iterations", static_cast<unsigned long>(joint_poses.size()),
           static_cast<unsigned long>(seed_states_.size()));

  if (joint_poses.empty())
  {
    logError("MoveitStateAdapter.getAllIK: Found 0 joint solutions out of %lu iterations", static_cast<unsigned long>(seed_states_.size()));
    return false;
  }
  else
  {
    logInform("MoveitStateAdapter.getAllIK: Found %lu joint solutions out of %lu iterations", static_cast<unsigned long>(joint_poses.size()),
              static_cast<unsigned long>(seed_states_.size()));
    return true;
  }
}
...
bool MoveitStateAdapter::getFK(const std::vector<double>& joint_pose, Eigen::Affine3d& pose) const
{
  ...
  if (isValid(joint_pose))
  {
    ...
    {
      logError("MoveitStateAdapter.getFK: Robot state does not recognize tool frame: %s", tool_frame_.c_str());
      rtn = false;
    }
  }
  else
  {
    logInform("MoveitStateAdapter.getFK: Invalid joint pose passed to get forward kinematics");
    rtn = false;
  }
...
bool MoveitStateAdapter::isValid(const std::vector<double>& joint_pose) const
{
  ...
  {
    logError("Joint pose does not satisfy positional bounds");
    return false;
  }
